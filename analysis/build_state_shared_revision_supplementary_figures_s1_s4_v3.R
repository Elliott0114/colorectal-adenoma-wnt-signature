...
p3b <- ggplot(fine_diagnostics, aes(k, silhouette, colour = broad_state)) +
  ...
  labs(x = "Substates per broad state", y = "Normal-reference silhouette") +
  theme_cb() +
  theme(legend.position = "top")
...
p3c <- ggplot(fine_effects, aes(estimate, scope, colour = model, shape = model)) +
  ...
  labs(x = "Substate-adjusted adenoma effect", y = NULL) +
  theme_cb() +
  theme(legend.position = "top")
...
p3d <- ggplot(fraction, aes(k, within_fraction, colour = partition, group = partition)) +
  ...
  labs(x = "Substates per broad state", y = "Within-substate fraction") +
  theme_cb() +
  theme(legend.position = "top")
...
decomp_primary <- decomposition %>%
  ...
  mutate(
    component = factor(component, levels = c("composition", "within"), labels = c("Substate composition", "Within substates")),
    partition = factor(partition, levels = c("discovery", "validation"), labels = c("Discovery", "Donor-disjoint validation"))
  )
...
p3h <- ggplot(dslab_stats, aes(estimate, "State-shared response")) +
...
save_supplement(fig_s3, 2, "substate_and_composition_sensitivities", 240)
...
write_source(dslab, source_dir, "figureS2g_dslab_common_composition.tsv")
