...
pathways <- read_tsv(file.path(
  result_root, "interpretation", "pathway_competitive_enrichment_all.tsv.gz"
))
...
# d. Selected pathways on one signed scale.
desired_pathways <- c(
...
pathway_plot <- pathways %>%
  filter(gene_set %in% desired_pathways) %>%
  mutate(
    label = pathway_labels[gene_set],
    signed_evidence = ifelse(Direction == "Up", 1, -1) *
      pmin(-log10(pmax(FDR, 1e-20)), 20),
    label = factor(label, levels = rev(unname(pathway_labels)))
  )

p1d <- ggplot(pathway_plot, aes(signed_evidence, label, colour = Direction)) +
  geom_vline(xintercept = 0, linewidth = 0.35, colour = colour[["line"]]) +
  geom_segment(aes(x = 0, xend = signed_evidence, yend = label), linewidth = 0.8) +
  geom_point(size = 2.4) +
  scale_colour_manual(values = c("Down" = colour[["down"]], "Up" = colour[["up"]])) +
  scale_x_continuous(
    breaks = c(-20, -10, 0, 5), labels = c("20", "10", "0", "5")
  ) +
  labs(x = "Downregulated  ←  enriched evidence  →  Upregulated", y = NULL) +
  theme_journal() +
  guides(colour = "none") +
  theme(axis.line.y = element_blank(), axis.ticks.y = element_blank())

# e. Directional anchor genes across all three broad states.
anchor_order <- c(
...
anchor <- common %>%
  ...
  tidyr::pivot_longer(-gene, names_to = "state", values_to = "effect") %>%
  mutate(
    state = sub("posterior_mean_", "", state),
    gene = factor(gene, levels = rev(anchor_order)),
    state = factor(state, levels = c("ABS", "GOB", "TAC"))
  )
limit <- max(abs(anchor$effect), na.rm = TRUE)

p1e <- ggplot(anchor, aes(state, gene, fill = effect)) +
  geom_tile(colour = "white", linewidth = 0.45) +
  geom_text(aes(label = sprintf("%.1f", effect)), family = font, size = 2.0) +
  scale_fill_gradient2(
    low = colour[["down"]], mid = "white", high = colour[["up"]],
    midpoint = 0, limits = c(-limit, limit), name = "Posterior effect"
  ) +
  labs(x = NULL, y = NULL) +
  ...
  theme(
    ...
    axis.text = element_text(size = 6.5), plot.margin = margin(2, 2, 2, 2, "mm")
  )

# f. Whole-donor stability of the complete common ranking.
...
figure <- p1a /
  ...
  (p1d | p1e) /
  p1f +
  plot_layout(heights = c(1.25, 1, 1.05, 0.72)) +
...
write_source(pathway_plot, "figure1d_pathways.tsv")
write_source(anchor, "figure1e_anchor_genes.tsv")
write_source(leaveout_common, "figure1f_leaveout.tsv")
